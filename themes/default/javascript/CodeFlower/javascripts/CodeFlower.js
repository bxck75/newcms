var CodeFlower = function(selector, w, h) {
  ...
  this.force = d3.layout.force()
    ...
    //.linkStrength(1) // ?
    //.friction(.9) // slowdonw
    //.charge(-30) // mass / antimass, neg repulse, pos  attract.
    //.theta(0.8) // linked nodes: link between these specific items 
    //.gravity(0.1) // gravity force of the gravity field at it's point of
...
CodeFlower.prototype.update = function(json) {
  ...
  this.force
    //.linkDistance(20) // ?
    //.linkStrength(1) // ?
    .friction(0.9) // slowdonw
    //.gravity(Math.atan(total / 50) / Math.PI * 0.4)
    //.charge(-30) // mass / antimass, neg repulse, pos  attract.
    //.theta(0.1) // linked nodes: link between these specific items 
    .gravity(0.2) // gravity force of the gravity field at it's point of
    .nodes(nodes)
    .links(links)
    .start();
  ...
  this.node.transition()
    .attr("r", function(d) { return d.children ? 1.5 : Math.pow(d.size, 2/5) || 1; });
...
CodeFlower.prototype.mouseover = function(d) {
  this.text.attr('transform', 'translate(' + d.x + ',' + (d.y - 5 - (d.children ? 4.5 : Math.sqrt(d.size) / 5)) + ')')
